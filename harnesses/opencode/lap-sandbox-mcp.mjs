 ...
 * `sandbox_files["setup.sh"]` on the agent record, and every future sandbox
 * provisioned for that agent executes it automatically.
 *
 * Env contract:
...
async function callUpdateSandboxSetup(env, input) {
  ...
  // GET current sandbox_files, replace or upsert the setup.sh entry
  ...
  return {
    isError: false,
    text: "setup.sh saved to sandbox_files. New sandboxes for this agent will run it automatically.",
  };
}
...
async function callGetSandboxSetup(env, input) {
  ...
  try {
    const script = Buffer.from(entry.content, "base64").toString("utf-8");
    return { isError: false, text: script };
  } catch {
...
const TOOLS = [
  {
    ...
    description:
      "Update the setup.sh script that runs automatically every time a new sandbox is provisioned for this agent. Use this to install dependencies, configure environment variables, clone repos, or do any other one-time setup that your tasks repeatedly require. The script is stored in sandbox_files and runs before the agent starts working in a new session.",
    inputSchema: {
      type: "object",
      properties: {
        script: {
          type: "string",
          description: "Bash script content to store as setup.sh. Will be executed automatically on each new sandbox provisioned for this agent.",
        },
        agent_id: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Retrieve the current setup.sh script configured for this agent. Call this to inspect what setup will run in new sandboxes before deciding whether to update it.",
    inputSchema: {
      type: "object",
      properties: {
        agent_id: {
...
console.error(
  env
    ...
    : `[lap-sandbox-mcp] disabled — missing env: ${missing.join(", ")}. lap_update_sandbox_setup will NOT be exposed.`,
);
