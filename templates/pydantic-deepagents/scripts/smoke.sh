...
# Pydantic Deep Agents template server.
...
agent_json=$(curl -s "${HDR[@]}" -X POST "$BASE/v1/agents" -d "$(cat <<JSON
{
  "name": "Smoke Test",
  "model": "$MODEL",
  "system": "You are a concise assistant. Use available tools when the user asks you to inspect files."
}
JSON
)")
echo "$agent_json"
...
sse_tmp="$(mktemp -t pydantic-deepagents-smoke-sse.XXXXXX)"
...
{
  "events": [
    { "type": "user.message", "content": [ { "type": "text", "text": "Use your file tools to list . and find markdown files. Then summarize briefly." } ] }
  ]
}
JSON
)")
