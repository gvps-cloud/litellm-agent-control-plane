...
import { z } from "zod";
import type { ServerEnv } from "@/api/types";
...
const EnvSchema = z.object({
  ...
  // src/api/auth/agent-token.ts). Tokens issued at pod-spawn time are
  ...
  // production cluster — see src/api/k8s.ts loadKubeConfig().
  ...
  DAYTONA_MEMORY: z.coerce.number().int().positive().optional(),
